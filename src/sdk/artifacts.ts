...
export class Artifacts {
    ...
    async getOASSummary(
        req: operations.GetOASSummaryRequest,
        config?: AxiosRequestConfig
    ): Promise<operations.GetOASSummaryResponse> {
        if (!(req instanceof utils.SpeakeasyBase)) {
            req = new operations.GetOASSummaryRequest(req);
        }
        ...
        const res: operations.GetOASSummaryResponse = new operations.GetOASSummaryResponse({
            statusCode: httpRes.status,
            contentType: responseContentType,
            rawResponse: httpRes,
        });
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(responseContentType, `application/json`)) {
                    res.oasSummary = utils.objectToClass(JSON.parse(decodedRes), shared.OASSummary);
                } else {
                    throw new errors.SDKError(
                        "unknown content-type received: " + responseContentType,
                        httpRes.status,
